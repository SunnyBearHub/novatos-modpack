...
class PlayState extends MusicBeatState
{
	...
	// Debug buttons quitar el chart editor
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "Botplay (Cheater Mode)", 32);
		...
		if(ClientPrefs.hitsoundVolume > 0) CoolUtil.precacheSound('note_click');
  ...
  public function clearShaderFromCamera(cam:String){
	...
	nps = Math.floor(notesHitArray.length / 2);
		...
		if(ratingName == '?') { //wtf ayop
			scoreTxt.text = 'NPS: ' + nps + ' | Score: ' + songScore + ' | Misses: ' + songMisses + ' | Combo: ' + combo + ' | All Notes Hit: ' + songHits + ' | Rating: ' + ratingName;
			healthCounter.text = 'Health: 50%';
		} else {
			scoreTxt.text = 'NPS: ' + nps + ' | Score: ' + songScore + ' | Misses: ' + songMisses + ' | Combo: ' + combo + ' | All Notes Hit: ' + songHits + ' | Rating: ' + ratingName + ' (' + Highscore.floorDecimal(ratingPercent * 100, 2) + '%)' + ' - ' + ratingFC;//peeps wanted no integer rating
			healthCounter.text = 'Health: ' + Math.round(health * 50) + '%'  ;
		}
		if(cpuControlled) {
			scoreTxt.text = 'Botplay On | Noob';
			FlxTween.tween(healthCounter, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(judgementCounter, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(iconP1, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(iconP2, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(healthBar, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(healthBarBG, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
			FlxTween.tween(judgementCounter, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 3});
		}
		if(practiceMode) {
		...
		if (FlxG.keys.anyJustPressed(debugKeysChart) && !endingSong && !inCutscene) //quit Chart Editor
		{
			openChartEditor();
			//health = 0;
			//trace("RESET = True");
		...
		if (!ClientPrefs.tabi)
			...
			if (health > ClientPrefs.tabimax)
		    	...
		    	//scoreTxt.color = CoolUtil.smoothColorChange(scoreTxt.color, FlxColor.fromRGB(51, 255, 153), 0.3); not used
			}
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			{
				trace('NOS VAMOS AL FREEPLAY??');  // 				trace('WENT BACK TO FREEPLAY??');
				cancelMusicFadeTween();
				CustomFadeTransition.nextCamera = camOther;
				if(FlxTransitionableState.skipNextTransIn) {
					CustomFadeTransition.nextCamera = null;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		switch (daRating)
		{
			case "shit": // shit
				totalNotesHit += 0;
				shits++;
				rating.color = 0x5c2924;
			case "bad": // bad
				totalNotesHit += 0.5;
				bads++;
				rating.color = 0xa30a11;
			case "good": // good
				totalNotesHit += 0.75;
				goods++;
				rating.color = 0x14cc00;
			case "sick": // sick
				totalNotesHit += 1;
				sicks++;
				rating.color = 0x00ffff;
			case "epic": // epic
				totalNotesHit += 98;
				epic++;
				rating.color = 0xB200FF;	
		}
		...
		if (currentTimingShown.alpha != 1)
		        ...
		        FlxTween.tween(currentTimingShown, {alpha: 0}, 1, {ease: FlxEase.circOut, startDelay: 0.2});
		...
		if (!cpuControlled && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				if (sortedNotesList.length > 0) {
					...
					noteMissPress(key);//if (curBeat % gfSpeed == 0 && !gf.stunned && gf.animation.curAnim.name != null && !gf.animation.curAnim.name.startsWith("sing")
	...
	override function beatHit()
	{
		...
		var funny:Float = (healthBar.percent * 0.01) + 0.01; //icon bounce here
...
var funny:Float = (healthBar.percent * 0.01) + 0.01;
	...
	public function RecalculateRating() {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (epic > 0) ratingFC = "EFC";
			if (sicks > 0) ratingFC = "SFC";
			if (goods > 0) ratingFC = "GFC";
			if (bads > 0) ratingFC = "FC";
			if (shits > 0) ratingFC = "Shit FC";
